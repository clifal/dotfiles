...
Node.js 20.18以降とnpmが使える場合は、Suikoと同じ対象へtextlintのAI文章preset（`@textlint-ja/ai-writing` と `ai-words-ja`）も実行する。プロジェクトのtextlint設定が同じpresetを有効にしている場合は、ロックされた依存関係と既存の許可設定を尊重して、そのプロジェクトの検査コマンドを一度だけ使う。設定されていない場合は、Skillディレクトリを基準に次の同梱スクリプトを使う。

```sh
sh <skill-directory>/scripts/run-textlint-ai-writing.sh <file>
```

この検査は固定したtextlintと2つのpresetをnpmの一時環境で実行し、プロジェクトの依存関係や設定を変更しない。`--fix`は使わない。ルール構成はSkill同梱の`scripts/textlint-ai-writing.rc.json`で固定し、`--config`で明示的に読ませる。そのためプロジェクトの`.textlintrc`は探索されない。`ai-words-ja`の`no-short-topic-comma`（主題を短く示しただけで打つ読点）は本来既定で無効だが、この設定で有効にしてある。指摘のruleIdは`@textlint-ja/ai-writing/`と`ai-words-ja/`で分かれるので、どちらのpresetが指したかはそこで判別する。終了コード1でもJSONに`messages`があれば検出結果として扱う。Node/npmがない、ネットワークから取得できない、対象形式を処理できないなどJSONを取得できない場合は、理由を一言示してSuikoの検査だけを続ける。textlintの結果は別の検査結果として扱い、Suikoの自然度スコア、baseline、finding件数には加えない。
...
この分類の例外は`ai-words-ja`の指摘で、こちらは残さず全件直す。判断するのは直すかどうかではなく、どう直すかである。詳細は「既存の校正工程との境界」に書いた。
...
既存の校正設定や用語辞書はプロジェクト規約として尊重し、Suikoで置き換えない。Suikoは形態素列、文書内の反復、構造、読解負荷を受け持つ。textlintのAI文章presetのうち`@textlint-ja/ai-writing`はMarkdown AST上の定型表現、強調、箇条書き、コロン接続を補い、`ai-words-ja`はAIが書いた日本語に出やすい語や言い回しを形態素解析で補う。`ai-words-ja`の指摘は文脈に合っていても残さず、全件直す。語を別の語へ置き換えるだけでは意味や技術的な正確さが崩れる場合は、その語を残さずに文を組み替える。組み替えても原稿の主張、事実、数値、固有名詞は変えない。両方が同じ箇所を指した場合は、判断台帳では一つの修正として扱い、同じ修正を二重に適用しない。一般的な表記規則や製品名の正規化は既存の工程へ残す。
